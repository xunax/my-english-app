...
// 🚀 修正 1：測驗防呆升級，強制 10 題選擇題，並清除 Markdown 標記
export async function generateQuiz(context: string): Promise<QuizQuestion[]> {
  const promptText = `你是一位「極速英文家教」。只准輸出純 JSON，絕對不准有任何其他文字與 Markdown 標記（不要有 \`\`\`json）。
  內容：${context}
  ...
  const response = await ai.models.generateContent({
    model: "gemini-2.5-flash",
    contents: promptText, // 改用最單純的字串傳遞，避免複雜結構報錯
    config: {
...
export async function chatWithAI(message: string, history: { role: "user" | "model"; text: string }[]) {
  try {
    ...
    const finalPrompt = transcript.length > 0 
      ? `【歷史對話紀錄】\n${transcript}\n\n【最新問題】\n學生：${message}\n\n請根據上述紀錄與最新問題，給出老師的回覆（直接回答即可，開頭不需加「老師：」）：`
      : message;

    const response = await ai.models.generateContent({
      model: "gemini-2.5-flash",
      contents: finalPrompt, // 傳入純文字，100% 免疫 ContentUnion 報錯
      config: {
