      ...
      <div className="flex flex-col h-full p-6 overflow-y-auto pb-28">
        ...
        {showExplanation && (
          <div className="mt-6 p-5 bg-stone-100 rounded-2xl">
            ...
            <button onClick={() => { if (quizIndex < currentQuiz.length - 1) { setQuizIndex(quizIndex + 1); setShowExplanation(false); } else { const score = currentQuiz.reduce((acc, q) => acc + (userAnswers[q.id] === q.correct_answer ? 1 : 0), 0); setQuizScore(score); } }} className="w-full mt-4 py-3 bg-stone-900 text-white rounded-xl font-bold">下一題</button>
          </div>
        )}
      </div>
    );
